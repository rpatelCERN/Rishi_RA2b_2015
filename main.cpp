...
void muContributionAllBins(std::vector<float>&SmearedNJ,std::vector<float>&SmearedMHT,
...
int main(int argc, char *argv[]){
  ...
  //  tempHistoWeight->Sumw2();
      ...
      //  std::cout<<"NJets w/o Muon Jet "<<NJets<<std::endl;
        ...
        for(int m=0; m<Mu.size();++m){
         ...
         //   TH3F*tempH=(TH3F*)tempHisto->Clone("tempH");
            ...
            muContributionAllBins(SmearedNJets, SmearedMHT,SmearedHT,SmearedNB,SmearedNoB, SmearedWeight,SmearedNBWeight, SmearedNoBWeight, tempHistoBbins);
           // delete tempHisto;
            //fill count
            for(int i=1; i<=3; ++i)
                for(int j=1; j<=3; ++j)
        ...
        //tau Event:
            ...
            for(int T=0; T<GenTauHad.size(); ++T){
              ...
              //  if(evt->ht()>500 && evt->nJets()>=4 && mockMHT.Pt()>200){
                ...
                //check here if the btag eff will work:
                std::vector<TLorentzVector>MatchedJets=evt->MatchTauJets_();
                std::vector<float>MatchedTauJetB=evt->TauJetsBtagged_();
                HadTauYields->Fill(evt->nJets(),evt->mht(),evt->ht());
                
                for(int i=0; i<MatchedJets.size();++i){
                    GenTauPt->Fill(MatchedJets[i].Pt());
                    if(MatchedTauJetB[i]>0.814)GenTauPtB->Fill(MatchedJets[i].Pt());
                }
    ...
    TFile*f2=new TFile("outputID.root", "RECREATE");
    f2->cd();
    MuPredictedHT->Write();
